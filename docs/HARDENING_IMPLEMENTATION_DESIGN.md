...
`manifest.csv`는 변수명, 파일명, 원본 레이어명, kind, units, resampling을 기록한다. 파일 경로는 bundle 내부의 basename으로 기록한다. `grid.json`은 기준 CRS, 픽셀 크기, 범위, 연속형 NoData, run id를 기록한다.
...
POSIX의 `mkdtemp()`는 기본적으로 owner 전용 `0700` 디렉터리를 만든다. 현재 구현은 이 mode를 게시 후에도 유지하므로 로컬 개인 작업에는 안전하지만, 그룹 공유 폴더에서 동료가 즉시 읽어야 하는 운영에는 별도 권한 정책이 필요하다. 부모 디렉터리 mode·umask·setgid를 어떻게 상속할지는 후속 설계 대상으로 남긴다.
...
6. `QgsRasterLayer`로 결과를 열어 최소 한 band, 양의 width·height, 읽을 수 있는 provider와 샘플 block을 확인한다.
7. 확인 가능한 경우 기준 CRS auth id와 출력 CRS auth id가 같은지 검사한다.

현재 검증은 “파일이 QGIS에서 열리고 기본 격자와 샘플 픽셀을 읽을 수 있는가”에 초점을 둔다. 출력 extent, 정확한 pixel size, 모든 band의 전수 픽셀, NoData 값까지 다시 대조하는 검증은 아직 구현하지 않았다.
...
현재 `_ErrorTrackingFeedback`는 `reportError()`로 전달된 모든 메시지를 게시 차단 사유로 취급한다. 이는 오류가 난 부분 결과를 내보내는 것보다 보수적인 정책이지만, 일부 QGIS/GDAL 조합이 exit code 0인 warning도 같은 채널로 전달하면 정상 결과를 거부할 수 있다. 향후에는 fatal flag, subprocess exit code와 warning 분류를 함께 보존해 안전성을 낮추지 않으면서 false reject를 줄여야 한다.
...
| `tools/atomic_output.py` | marker 기반 staging 생성·안전 정리·동일 파일시스템 디렉터리 게시를 제공한다. QGIS 의존성이 없다. |
| `tools/align_export_dialog.py` | 필수 출력 폴더, task manager 기반 warp, 취소·실패 분기, 기본 출력 검증, manifest 작성, 원자적 게시, 프로젝트 등록 rollback을 구현한다. |
| `tests/test_atomic_output.py` | unmarked 삭제 거부, staging 정리, bundle 게시, marker 제거 실패, 기존 결과 보호, unmarked 게시 거부를 검증한다. |
| `tests/test_release_identity.py` | badge, 상태 플래그, BibTeX, CFF, 날짜, URL, category, duplicate, changelog 경계를 검증한다. |
| `tests/test_align_export_qgis.py` | 실제 QGIS/GDAL 환경에서 활성 작업 취소와 valid-looking 부분 TIFF의 provider 오류 차단을 검증하며, PyQGIS가 없으면 skip한다. |
...
QGIS 3.44.8의 실제 GDAL provider에서 가장 위험한 두 경로를 자동 통합 테스트로 확인했다.

- 100M-cell 출력으로 확장되는 warp 중 실제 진행창 취소 버튼을 클릭했을 때 약 0.104초 안에 `_Cancelled`로 종료되고 완전한 출력이 남지 않았다.
- QGIS에서는 열리지만 타일 일부가 잘린 GeoTIFF를 처리했을 때 GDAL이 부분 출력도 만들었으나, feedback의 `Process returned error code 1`을 잡아 `RuntimeError`로 게시를 차단했다.

일반 Python CI에서는 PyQGIS/GDAL이 없으므로 이 두 테스트가 명시적으로 skip된다. QGIS 환경에서는 아래 시나리오 중 핵심 두 항목이 자동화되었고, 나머지는 릴리스 전 수동·확장 통합 검증 대상으로 남는다.
...
| 공급자 오류 | **QGIS 3.44.8 자동 검증 완료** — valid-looking 부분 출력도 오류 feedback으로 차단 |
...
2. 출력 width, height, extent, pixel size, band별 NoData를 기준 격자와 엄밀히 비교하는 검증을 추가한다.
3. 애플리케이션 비정상 종료 후 유효 marker를 가진 오래된 staging을 목록화하고 사용자의 확인을 받아 정리하는 복구 UI를 설계한다.
4. 네트워크 드라이브, 외장 디스크, 권한이 제한된 폴더에서 directory rename의 실제 원자성과 오류 메시지를 검증한다.
5. 필요하면 bundle 완전성 파일에 파일 크기나 checksum을 기록하되, 대형 래스터의 비용을 먼저 측정한다.
6. 전체 CFF 스키마 검증은 릴리스 환경에서 이용 가능한 공식 도구 도입 여부를 별도로 검토한다. 런타임 의존성에는 추가하지 않는다.
7. `STABILITY.md`의 `work/*`와 자동화의 `agent/*` 브랜치 규칙을 하나의 운영 규칙으로 정리한다.
8. 현재 UI·README pictograph 정적 검사의 허용 범위를 유지하면서 문서 예시와 학술 기호를 오탐 없이 분리한다.
9. 픽셀 아이콘 공통 팔레트, 16×16·24×24·32×32 변형, 명명 규칙을 작은 디자인 시스템으로 문서화한다.
10. Align & Export에서 검증된 원자적 게시 helper를 다른 장시간 실행 도구에 적용할지 도구별 위험도를 평가한다.
11. full flake8의 기존 스타일 부채를 기능 변경과 분리한 작은 커밋으로 점진적으로 줄인다.
12. 출력 경로 변경을 README의 사용 예시와 향후 migration note에 더 명시한다.
13. GDAL warning과 실제 실패를 구분할 수 있도록 feedback의 fatal flag와 subprocess exit code를 구조적으로 기록한다.
14. 그룹 공유 폴더에서 게시 bundle에 적용할 POSIX mode·umask·setgid 상속 정책을 정하고 테스트한다.
...
- QGIS에서 정상 정렬, warp 중 취소, 공급자 실패, 프로젝트 등록 실패를 최소 한 번씩 검증한다.
...
현재 정적·단위 검증과 QGIS 3.44.8의 활성 취소·provider 실패 통합 검증은 완료했다. 프로젝트 등록 실패, 창 닫기·unload·종료, 테마·HiDPI, 지원 환경별 차이는 릴리스 전 확장 확인 작업으로 남아 있다. 따라서 이 문서의 완료 기준은 코드가 제공하는 보장과 배포 전 반드시 수행해야 할 확인 작업을 함께 나타낸다.
