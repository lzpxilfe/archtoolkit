...
- AOI로 자를 때 AOI 경계 상자를 기준 격자에 바깥쪽으로 맞추므로 출력 셀은 항상 기준 셀과 일치합니다. 레이어 속성에서 지정한 NoData(파일에 없는 값)도 리샘플링에서 제외됩니다. CRS가 파일에도 레이어에도 없는 소스는 거부합니다.

### <img src="../icons/covariate.png" width="48" alt="" align="absmiddle"> 변수 상관/VIF 리포트 (Correlation & VIF)
- 상관행렬과 VIF를 계산합니다. 임계값 5·10은 참고선이며 리포트도 그렇게 표기합니다(O'Brien 2007: 표본 크기·효과 크기와 함께 판단). AOI는 폴리곤 내부 표본으로 적용되고, 표본 수는 셀 수를 넘지 않으며(초과 요청 시 전수), 리포트 머리에 범위·AOI·표본 수가 적힙니다. 취소하면 리포트를 만들지 않습니다.

### <img src="../icons/ahp.png" width="48" alt="" align="absmiddle"> AHP 입지적합도 (AHP Suitability)
- 쌍대비교(Saaty 1980) 가중치, 일관성비율, 계층형 AHP. 계층 모드에서는 전역가중치가 실제 계산에 쓰이며 메타데이터의 `weights_source`에 기록되고, 일관성비율도 그룹·하위 행렬의 값이 표시됩니다. 값이 하나뿐인 기준은 모드에 따라 처리되며(이익/비용은 0.5 고정, 경고), 데이터 범위 밖의 목표값·선호 범위는 범위 안으로 잘라 쓰고 그 사실을 알립니다. AOI 통계는 범위가 바뀌면 다시 계산되고, 래스터 밖 AOI는 거부됩니다. 목표·범위·리클래스 값과 쌍대비교 판단이 메타데이터에 남습니다. 자세한 사용법은 [AHP_GUIDE.md](AHP_GUIDE.md).
...
- AOI 안에서 폭·길이·개수·간격·내부 포함비율 조건으로 후보를 만들고 AOI 전체에 분산 배치합니다. `rank`는 점수순, `pick_order`는 배치 순서입니다. `slope_max_deg`는 트렌치 사각형 안 셀들의 실제 최대 경사입니다. AHP 값이 NoData인 후보와 DEM이 없는 후보는 제외되며 그 수가 메시지와 메타데이터에 적힙니다. 무덤 회피는 수치지형도 속성과 범례 어휘에 의존하므로 "대상 0건"이 무덤이 없다는 뜻은 아닙니다. 학술 방법이 아닌 조사 보조 규칙입니다.
