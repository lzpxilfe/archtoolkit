...
- 🧩 총 17개 도구를 하나의 플러그인 메뉴와 툴바 드롭다운으로 묶었습니다.
...
| 도구 | 핵심 기능 |
| --- | --- |
| `지형 분석 (Terrain Analysis)` | 경사, 사면방향, TRI, TPI, Roughness, Slope Position, 곡률(종단/횡단, Zevenbergen & Thorne 1987) 계산과 분류/스타일 적용, 곡률 해석 요약 |
| `DEM 파생 변수 일괄 생성 (Batch DEM Covariates)` | DEM 하나에서 예측모델용 파생변수(표고·경사·북향성/동향성·TRASP·TRI·거칠기·다중스케일 TPI)를 동일 기준 격자로 일괄 생성, GeoTIFF 스택+manifest 내보내기 |
