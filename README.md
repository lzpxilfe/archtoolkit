...
- 비용 누적은 8방향 격자 위에서 진행하므로 경로 방향이 45도 단위로 양자화됩니다. 이 때문에 누적 비용은 실제 측지선 대비 최대 약 8%까지 과대평가될 수 있습니다(격자 metrication 오차). `대각 이동 허용(8방향)`을 끄면 4방향 탐색이 되어 경로가 90도 단위로만 꺾이므로 과대평가가 최대 약 41%까지 커집니다. 따라서 절대 비용값을 보고할 때는 이 한계를 함께 밝히고 지점 간 상대 비교 위주로 해석하세요.
