...
<h4>채점 방식 고르기</h4>
<ul>
  <li><b>cost</b> — 작을수록 좋음 (하천까지 거리, 유적까지 거리).</li>
  <li><b>benefit</b> — 클수록 좋음 (누적 가시권, 조망).</li>
  <li><b>target</b> — 최적값이 중간에 있음 (예: 경사 8도가 가장 좋고 너무 평탄해도 급해도 나쁨).</li>
  <li><b>range</b> — 최적이 구간 (예: 해발 50-120 m가 모두 동등하게 좋음).</li>
  <li><b>reclass</b> — <b>범주형 전용</b> (지질 단위, 토지피복). 구간마다 점수를 직접 지정합니다.</li>
</ul>
<p style="background:#fff3e0; padding:8px; border:1px solid #ffe0b2;">
<b>범주형에 benefit을 쓰지 마세요.</b> 지질 코드에 benefit을 주면 "코드 14가 코드 1보다
14배 좋다"는 뜻이 됩니다. 코드 번호에는 순서가 없으므로 완전히 틀린 해석입니다.
반드시 <b>reclass</b>를 쓰세요.
</p>

<h4>CR(일관성비율)이 말하는 것</h4>
<p>
CR은 <b>판단이 서로 모순되는 정도</b>일 뿐, 가중치가 옳은지는 보지 않습니다.
"A&gt;B, B&gt;C, 그런데 C&gt;A" 같은 순환을 잡아냅니다. 쌍대비교를 하나도 입력하지 않으면
CR은 0.000이지만 그건 일관적이어서가 아니라 <b>아무 판단도 하지 않았기 때문</b>입니다.
</p>
<p>
...
<ul>
  ...
  <li><b>이 적합도를 예측모델(MaxEnt/GLM/RF)의 입력 변수로 넣지 마세요</b> — 순환 논증이 됩니다.
      모델에는 원재료 변수(경사·거리·지질)를 직접 넣고, 적합도는 모델의 비교 대상으로 쓰세요.</li>
</ul>

<p style="color:#455a64;">
자세한 설명(채점 모드별 점수표, CR 실측 예시, 계층형, 자주 하는 실수)은 플러그인 폴더의
<code>docs/AHP_GUIDE.md</code>에 있습니다.
</p>
"""
        try:
            plugin_dir = os.path.dirname(os.path.dirname(__file__))
